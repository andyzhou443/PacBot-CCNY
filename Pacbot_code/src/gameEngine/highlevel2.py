...
class HighLevel(rm.ProtoModule):
    def __init__(self, addr, port):
        ...
        self.path = copy.deepcopy(bfs(self.grid, (self.cur_dir, self.pacbot_pos[0], self.pacbot_pos[1]), [o, O]))
        self.isClose = False
        self.ghost_states = []
    ...
    def getPathToGhost(self, starting, original_path):
    ...
    def euclnDist(self, pointA, pointB):
    ...
    def gameRun(self):
        ...
        if self.path is None:
            ...
            self.path = copy.deepcopy(self.getPathToPellet((self.cur_dir, self.pacbot_pos[0], self.pacbot_pos[1])))
        # self.path = copy.deepcopy(bfs(self.grid, (self.cur_dir, self.pacbot_pos[0], self.pacbot_pos[1]), (self.state.pink_ghost.x, self.state.pink_ghost.y)))
        ...
        ghost_x, ghost_y = ghost[1][0], ghost[1][1]
        print("close to ghost: ", self.isClose)
        if(self.ghostsStates() and self.isClose):
            self.path = copy.deepcopy(bfs(self.grid, (dir,x,y), (ghost_x, ghost_y)))
            dir = self.path[self.i][0]
            x = self.pacbot_pos[0]
            y = self.pacbot_pos[1]
        # elif((not self.ghostsStates()) and self.isClose):
        #     self.path = copy.deepcopy(bfs(self.grid, (dir,x,y), (ghost_x-10, ghost_y-10)))
        #     dir = self.path[self.i][0]
        #     x = self.pacbot_pos[0]
        #     y = self.pacbot_pos[1]
